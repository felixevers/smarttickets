...
def getTicket(ticket, meeting, customer, seat, seats, price):
...
def create_pdf(ticket):
    if ticket and ticket.paid:
        ...
        return getTicket(ticket, meeting, customer, seat, seats, price)
...
def create_pdf_direct(customer_uuid):
    ...
    for ticket in TicketModel.query.filter_by(customer=customer_uuid):
        if ticket and not ticket.paid:
            ...
            htmls.append(getTicket(ticket, meeting, customer, seat, seats, price))

    return htmls

def create_all_pdf(meeting_uuid):
        htmls = []
        for ticket in TicketModel.query.filter_by(meeting_id=meeting_uuid):
            if ticket and not ticket.paid:
                customer = CustomerModel.query.filter_by(uuid=ticket.customer).first()
                meeting = MeetingModel.query.filter_by(uuid=ticket.meeting_id).first()
                seat = SeatModel.query.filter_by(uuid=ticket.seat_id).first()
...
def register_download(app: Flask):
    ...
    def render_download(uuid):
        ...
        pdf = pdfkit.from_string("".join(create_pdf(ticket)), False, options=options)
    ...
    def render_direct_download(token, uuid):
        ...
        pdf = pdfkit.from_string("".join(create_pdf_direct(uuid)), False, options=options)
    ...
    @app.route('/directAll/<token>/<uuid>')
    def render_download_all(token, uuid):
        session: Optional[SessionModel] = SessionModel.query.filter_by(uuid=token).first()

        if session is None:
            abort(400, "invalid token")

        pdf = pdfkit.from_string("".join(create_all_pdf(uuid)), False, options=options)
